...
| target_type別 | `screen`、`api`、`batch`、`db`、`external` で観点が切り替わる |
| test_level別 | `unit`、`integration`、`system` で観点が切り替わる |
...
```json
{
  ...
  "target_type": "batch",
  "test_level": "system",
  "spec_text": "日次で売上データを集計し、売上サマリテーブルを更新するバッチ。",
  "supplement": "異常終了時はログを確認し、原因を取り除いたうえで再実行する。"
}
```
...
```json
{
  ...
  "supplement": "認証失敗時はエラーメッセージを表示する。"
}
```
