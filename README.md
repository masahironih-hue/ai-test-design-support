...
Phase 1：ローカルMVPでは、仕様入力、LLM Mockによる生成、Markdown表示・コピー、SQLite履歴保存、履歴一覧、履歴詳細まで実装・確認済みです。Backendでは任意機能として `APP_LLM_PROVIDER=mock/openai` の切替に対応しています。
...
ただし、OpenAI API連携は任意機能であり、デフォルトはMock生成です。Amazon Bedrock連携は未実装です。AWS構成も、個人開発向けの低コスト検証構成であり、商用SaaS運用や高可用性構成を目的としたものではありません。
...
- モデル変更時はOpenAI公式の料金・品質・速度・利用制限を確認してください
...
- デフォルトはLLM Mockを使用しており、実際の外部LLM APIは呼び出していない
- OpenAI API連携は任意機能として実装済み
- Amazon Bedrock連携は未実装
- LLM Provider切替は `APP_LLM_PROVIDER=mock/openai` に対応
