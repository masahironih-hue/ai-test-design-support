...
加えて、Backend最小APIとして API Gateway HTTP API + Python Lambda による `POST /test-designs/generate` 相当のMock生成APIを追加します。
...
- DynamoDB履歴保存、履歴一覧API、履歴詳細APIは未AWS化として整理
...
AWS低コスト版では、Backend最小APIとして `POST /test-designs/generate` 相当のMock生成APIのみをAWS化します。

ただし、Frontend接続切替、DynamoDB履歴保存、履歴一覧API、履歴詳細APIは今回の対象外です。

そのため、CloudFrontで配信されるFrontendからAWS Backendを利用するには、後続で接続先切替と追加API実装が必要です。
...
AWS上のBackend APIは `POST /test-designs/generate` の単体疎通確認を目的とした最小構成であり、履歴保存と履歴APIは後続タスクで段階的に対応する。
...
今回のBackend最小APIでは、上記候補のうち API Gateway HTTP API、Python Lambda、CloudWatch Logs のみを作成し、DynamoDBは作成しない。
