...
Phase 2では、S3 + CloudFront による Frontend静的配信を検証済みです。

加えて、Backend最小APIとして API Gateway HTTP API + Python Lambda + DynamoDB によるMock生成APIと履歴保存APIを追加します。
...
- API Gateway HTTP API作成
- Python LambdaによるMock生成API作成
- `POST /test-designs/generate` の単体疎通確認
- DynamoDBによる履歴保存
- `GET /test-designs/histories`、`GET /test-designs/histories/{history_id}` route追加
- CloudWatch Logs保持期間7日設定
- 既存VPC / EC2 / Security Group / Route 53を使用・変更しないことの確認
- 対象S3 Bucketの中身を手動削除したうえでの `pnpm cdk destroy` 確認

検証後に削除する場合は、本プロジェクト用Stackのみを対象にします。

READMEやdocsには、API Gateway URL、CloudFront DomainName、S3 Bucket名などの実値を掲載しません。
...
| Backend API | API Gateway HTTP API + Python Lambda | Mock生成APIと履歴API |
| 履歴保存DB | DynamoDB | 生成履歴保存 |
...
### 7.4 Backend APIの現在の制約

AWS低コスト版では、Backend最小APIとして `POST /test-designs/generate` 相当のMock生成APIとDynamoDB履歴保存APIをAWS化します。

ただし、Frontend接続切替、OpenAI API連携、Amazon Bedrock連携、Cognito認証、本格API認証は今回の対象外です。

そのため、CloudFrontで配信されるFrontendからAWS Backendを利用するには、後続で接続先切替と認証・公開範囲制御の検討が必要です。
...
### 7.5 Phase 2 Backend最小API

今回追加するBackend最小APIは以下です。
...
- FastAPI on Lambda
- Frontend接続切替
- VPC、Subnet、Security Group、NAT Gateway、ALB、RDS、ECS Fargate、EKS、WAF

LambdaはVPCなし構成とし、既存VPC / EC2 / Security Group / IAM Role / Route 53設定は使用・変更しません。
...
## 9. Backend Serverless構成

Phase 2では、以下のServerless構成をBackend最小APIとして採用します。
...
| サービス | 用途 | 検討タイミング |
|---|---|---|
| API Gateway | Backend API公開 | Phase 2 Backend最小API |
| Lambda | Backend API実行 | Phase 2 Backend最小API |
| DynamoDB | 履歴保存 | Phase 2 Backend最小API |
| CloudWatch Logs | APIログ確認 | Phase 2 Backend最小API |
...
Backend APIはAPI Gateway HTTP API + Lambda + DynamoDBで最小構成を追加し、実LLM連携や認証は後続検討です。
```
...
API Gateway HTTP API + Lambda + DynamoDB による Backend 最小API
```

Backend API、DynamoDB、CloudWatch Logsは低コストなServerless構成として追加します。
...
AWS上のBackend APIは `POST /test-designs/generate`、`GET /test-designs/histories`、`GET /test-designs/histories/{history_id}` を対象とした最小構成であり、Frontend接続切替と実LLM連携は後続タスクで段階的に対応する。
...
今回のBackend最小APIでは、上記候補のうち API Gateway HTTP API、Python Lambda、DynamoDB、CloudWatch Logs を作成する。
