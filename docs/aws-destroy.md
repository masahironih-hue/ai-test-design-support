...
今回、DynamoDBは作成していないため削除対象に含めない。
...
今回のBackend最小APIではDynamoDBを作成しない。

そのため、Backend Stack削除時にDynamoDBテーブルや履歴データは削除対象に含まれない。

DynamoDB履歴保存を後続で追加した場合は、削除時のデータ消失と対象テーブル名を改めて明記する。
