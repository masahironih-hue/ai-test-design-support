...
このドキュメントは、AIテスト設計支援ツールの Phase 2：AWS低コスト版における、S3 + CloudFront Frontend静的配信用リソースと Backend最小APIリソースの削除手順を整理するためのものです。
...
Backend最小APIを削除する場合、削除対象Stackは `AiTestDesignSupportBackendApiStack` である。
...
Backend Stack削除時、DynamoDB Tableは `RemovalPolicy.DESTROY` により削除対象になる。保存済みの履歴データも削除される。

Frontend Stackを削除する場合は、別途 `AiTestDesignSupportFrontendStack` を明示する。
...
DynamoDBはBackend Stackで作成する。

Backend Stack削除時にDynamoDB Tableと履歴データは削除対象に含まれる。

削除前に、対象が本プロジェクト用Stackに含まれるDynamoDB Tableであることを確認する。README/docsへDynamoDB実テーブル名は記載しない。
...
- [ ] 削除対象Stack名が `AiTestDesignSupportBackendApiStack` である
...
- [ ] DynamoDB Tableが本プロジェクト用Stackの削除対象である
...
```text
Backend Stackは、Frontend Stackとは分けて削除できるようにする。
削除手順では、本プロジェクト用Stackと本プロジェクト用リソースのみを対象にする。
Backend Stack削除時はDynamoDB Tableと履歴データも削除される。
