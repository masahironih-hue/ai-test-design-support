...
| POST | `/api/test-designs` | テスト設計結果の生成と履歴保存 |
| GET | `/api/test-designs` | 生成履歴一覧の取得 |
| GET | `/api/test-designs/{id}` | 生成履歴詳細の取得 |
...
タイムゾーンの扱いは、MVPではローカル開発前提の単純な日時文字列とします。  
将来的にAWSデプロイや複数環境対応を行う場合は、UTC保存やタイムゾーン表示方針を見直します。
...
| `input_text` | string | 入力された仕様メモ |
| `supplemental_notes` | string | 補足事項 |
...
```bash
curl http://localhost:8000/api/test-designs
```

---

## 9. GET `/api/test-designs/{id}`
...
```bash
curl http://localhost:8000/api/test-designs/1
```
...
- `GET /health` が動作する
- `POST /api/test-designs` でテスト設計結果を生成できる
- 生成結果がSQLiteに保存される
- `GET /api/test-designs` で履歴一覧を取得できる
- `GET /api/test-designs/{id}` で履歴詳細を取得できる
